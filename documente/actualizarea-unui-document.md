...
## Actualizare scriptată

### Actualizarea unui singur document

Selectând un document după id, am putea actualiza o valoare a unui câmp.

```yaml
POST /produse/_update/12
{
  "script": {
    "source": "ctx._source.vizualizari++"
  }
}
```

Chiar putem atribui valori câmpurilor.

```yaml
POST /produse/_update/12
{
  "script": {
    "source": "ctx._source.exemplare = 34"
  }
}
```
